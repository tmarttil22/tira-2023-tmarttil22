...
public class Graph<T> {
   ...
   public Set<Vertex<T>> getVertices() {
      // keySet() returns the hashmaps keys, which are the vertices
   ...
   public void addEdge(Edge.EdgeType type, Vertex<T> source, Vertex<T> destination, double weight) {
      switch (type) {
         case DIRECTED:
            ...
            break;
         case UNDIRECTED:
            // adding 2 directed edges to-from AND from-to == 1 undirected edge
            Edge<T> edge1 = new Edge<>(source, destination, weight);
            edgeList.get(source).add(edge1);

            Edge<T> edge2 = new Edge<>(destination, source, weight);
            edgeList.get(destination).add(edge2);
            break;
      }
   }
   ...
   public List<Edge<T>> getEdges(Vertex<T> source) {
      return edgeList.getOrDefault(source, null);
      // chatGPT used to find a way to get a value from a hashmap with a key
   }
   ...
   public List<Vertex<T>> breadthFirstSearch(Vertex<T> from, Vertex<T> target) {
      ...
      Set<Vertex<T>> pastVertices = new HashSet<>();

      queue.add(from);
      pastVertices.add(from);
      ...
      while (!queue.isEmpty()) {
         ...
         List<Edge<T>> edges = edgeList.getOrDefault(current, null);
         for (Edge<T> edge : edges) {
            Vertex<T> neighborVertex = edge.getDestination();
            if (!pastVertices.contains(neighborVertex)) {
               queue.add(neighborVertex);
               pastVertices.add(neighborVertex);
            }
         }
      }
      return visited;
   ...
   public List<T> disconnectedVertices(Vertex<T> toStartFrom) {
      ...
      if (toStartFrom != null) {
         startVertex = toStartFrom;
      } else { // set startVertex to the next value in the map
         startVertex = edgeList.keySet().iterator().next();
      }

      // with the target as null, the whole graph is iterated through
      List<Vertex<T>> visited = breadthFirstSearch(startVertex, null);
   ...
   public boolean isDisconnected(Vertex<T> toStartFrom) {
      ...
      List<Vertex<T>> visited = breadthFirstSearch(toStartFrom, startVertex);
